...
if __name__ == "__main__":
    ...
    f1 = m.start_screen()
    if f1:
        mode_game = m.second_screen() # тут режим игры и размер карты
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
